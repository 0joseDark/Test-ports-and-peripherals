...
# Função para capturar a saída do comando `usbinfo` e obter informações dos dispositivos USB
def obter_informacoes_usb():
    try:
        # Executa o comando `usbinfo` no sistema e captura a saída
        resultado = subprocess.run(['usbinfo'], capture_output=True, text=True)
        return resultado.stdout.splitlines()  # Retorna a saída como uma lista de strings (linhas)
    except FileNotFoundError:
        return ["Erro: 'usbinfo' não encontrado no sistema."]  # Caso o comando não seja encontrado
...
def atualizar_lista_dispositivos():
    ...
    if not informacoes_usb:  # Caso não haja dispositivos ou erro na execução
        ...
        print("Informações sobre dispositivos USB:")
